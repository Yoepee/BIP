...
import axios from 'axios';
  ...
  export const __getWeather = createAsyncThunk(
    ...
    async (payload, thunkAPI) => {
        try {
            const data =  await axios.get(process.env.REACT_APP_SERVER_HOST+`/api/weather?coordinate=${payload}`,{
                headers: {
...
export const weather = createSlice({
    ...
    extraReducers: {
        ...
        [__getWeather.fulfilled]: (state, action) => {
          ...
          state.data.push(...action.payload.data); // Store에 있는 todos에 서버에서 가져온 todos를 넣습니다.
        },
